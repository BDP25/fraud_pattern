...
class DoctorFeatureExtractor:
    ...
    def extract_features(self, doctor_id, doctor_df):
        ...
        # extract all sequences for transformer model
        all_sequences = []
        for patient_id, patient_df in df.groupby('patient_id'):
            treatments = patient_df.sort_values('date')['treatment_code'].tolist()
            if len(treatments) > 1:  # Only include sequences with at least 2 treatments
                all_sequences.append(treatments)
    ...
    def prepare_sequence_data(self, min_seq_length=2):
        ...
        # Group sequences by specialty
        for doctor_id, features in self.doctor_features.items():
            specialty = features.get('specialty', 'unknown')
            
            for seq in features.get('all_sequences', []):
                if len(seq) >= min_seq_length:
                    specialty_sequences[specialty].append(seq)
                    doctor_sequences[doctor_id].append(seq)
        
        return specialty_sequences, doctor_sequences
    
    def save_features(self, path="./data/doctor_features.pkl"):
        with open(path, 'wb') as f:
            pickle.dump(self.doctor_features, f)  # Only save doctor_features
...
def main():
    ...
    extractor = DoctorFeatureExtractor()
    ...
    if os.path.exists(features_file):
        ...
        dataset = DoctorDataset(folder_path, columns=columns)
